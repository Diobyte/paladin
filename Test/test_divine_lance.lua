-- Test Divine Lance cooldown behavior
...
local spells = require('spells/divine_lance')
...
local target = { get_position = function() return { x = 0, y = 0, z = 0 } end, is_elite = function() return false end }
...
if not spells.logics(target) then
    print('TEST FAIL: divine lance first cast failed')
    os.exit(1)
end
TIME_NOW = 0.05
if spells.logics(target) then
    print('TEST FAIL: divine lance allowed early recast')
    os.exit(2)
end
TIME_NOW = 0.2
if not spells.logics(target) then
    print('TEST FAIL: divine lance did not cast after delay')
    os.exit(3)
end

print('TEST PASS: divine_lance cooldown behavior')
os.exit(0)
