-- Test Paladin Evade cooldown behavior
...
local spells = require('spells/paladin_evade')
...
local target = { get_position = function() return { x = 1, y = 0, z = 0 } end, is_elite = function() return false end }
...
if not spells.logics(target) then
    print('TEST FAIL: paladin evade first cast failed')
    os.exit(1)
end
TIME_NOW = 0.05
if spells.logics(target) then
    print('TEST FAIL: paladin evade allowed early recast')
    os.exit(2)
end
TIME_NOW = 0.2
if not spells.logics(target) then
    print('TEST FAIL: paladin evade did not cast after delay')
    os.exit(3)
end

print('TEST PASS: paladin_evade cooldown behavior')
os.exit(0)
