-- Test Brandish cooldown behavior
...
local spells = require('spells/brandish')
...
if not spells.logics(target) then
    print('TEST FAIL: brandish first cast failed')
    os.exit(1)
end
TIME_NOW = 0.05
if spells.logics(target) then
    print('TEST FAIL: brandish allowed early recast')
    os.exit(2)
end
TIME_NOW = 0.2
if not spells.logics(target) then
    print('TEST FAIL: brandish did not cast after delay')
    os.exit(3)
end

print('TEST PASS: brandish cooldown behavior')
os.exit(0)
