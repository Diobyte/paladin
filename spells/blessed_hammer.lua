...
local menu_elements =
{
    ...
    self_timer          = checkbox:new(false, get_hash(my_utility.plugin_label .. "blessed_hammer_self_timer")),
    self_timer_delay    = slider_float:new(0.01, 2.0, 0.05, get_hash(my_utility.plugin_label .. "blessed_hammer_self_timer_delay")),
}
...
local function logics(target)
    ...
    if menu_elements.self_timer:get() then
        ...
        -- Rotation mode: use main spell_allowed check with hardcoded delay
    ...
    if cast_spell.self(spell_data.blessed_hammer.spell_id, 0) then
        ...
        if menu_elements.self_timer:get() then
            ...
            console.print("Blessed Hammer (Self-Timed) - Delay: " .. string.format("%.2f", menu_elements.self_timer_delay:get()) .. "s");
        else
            next_time_allowed_cast = current_time + my_utility.spell_delays.blessed_hammer;
            console.print("Blessed Hammer (Rotation)");
        end
        return true;
    end;
