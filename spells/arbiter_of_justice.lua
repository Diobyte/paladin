...
local menu_elements =
{
    ...
    self_timer          = checkbox:new(false, get_hash(my_utility.plugin_label .. "arbiter_of_justice_self_timer")),
    self_timer_delay    = slider_float:new(0.01, 2.0, my_utility.spell_delays.arbiter_of_justice, get_hash(my_utility.plugin_label .. "arbiter_of_justice_self_timer_delay")),
}

local function menu()
    if menu_elements.tree_tab:push("Arbiter of Justice") then
        menu_elements.main_boolean:render("Enable Arbiter of Justice", "")
...
local function logics(target)
    ...
    if cast_spell.position(spell_data.arbiter_of_justice.spell_id, target:get_position(), 0) then
        ...
        if menu_elements.self_timer:get() then
            ...
            console.print("Arbiter of Justice (Self-Timed) - Delay: " .. string.format("%.2f", menu_elements.self_timer_delay:get()) .. "s");
        else
            next_time_allowed_cast = current_time + my_utility.spell_delays.arbiter_of_justice;
            console.print("Arbiter of Justice (Rotation)");
        end
        return true;
    end;
