...
local menu_elements =
{
    ...
    targeting_mode      = combo_box:new(0, get_hash(my_utility.plugin_label .. "aegis_targeting_mode")),
    min_target_range    = slider_float:new(0, 5, 0, get_hash(my_utility.plugin_label .. "aegis_min_target_range")),
}

local function menu()
    if menu_elements.tree_tab:push("Aegis") then
        menu_elements.main_boolean:render("Enable Aegis", "")
...
local function logics(target)
    ...
    if cast_spell.self(spell_data.aegis.spell_id, 0) then
        ...
        console.print("Cast Aegis - Target: " .. my_utility.targeting_modes[menu_elements.targeting_mode:get() + 1]);
        return true;
    end;
