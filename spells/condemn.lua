...
local menu_elements =
{
    ...
    min_target_range = slider_float:new(0, max_spell_range - 1, 0,
        get_hash(my_utility.plugin_label .. "condemn_min_target_range"), 1),
...
local function menu()
    if menu_elements.tree_tab:push("Condemn") then
        ...
        if menu_elements.main_boolean:get() then
            ...
            menu_elements.min_enemy_count:render("Min Enemy Count", "Minimum number of enemies to pull", 1)
...
local function logics(target)
    ...
    if min_enemy_count > 1 and not (force_priority and is_priority) then
        local enemies = actors_manager.get_enemy_npcs()
        local count = 0
        local range = 8.0 -- Condemn pull radius
