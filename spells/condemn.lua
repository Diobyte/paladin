...
local menu_elements =
{
    ...
    self_timer          = checkbox:new(false, get_hash(my_utility.plugin_label .. "condemn_self_timer")),
    self_timer_delay    = slider_float:new(0.01, 2.0, my_utility.spell_delays.condemn, get_hash(my_utility.plugin_label .. "condemn_self_timer_delay")),
}
...
local function logics(target)
    ...
    if cast_spell.self(spell_data.condemn.spell_id, 0) then
        ...
        if menu_elements.self_timer:get() then
            ...
            console.print("Condemn (Self-Timed) - Delay: " .. string.format("%.2f", menu_elements.self_timer_delay:get()) .. "s");
        else
            next_time_allowed_cast = current_time + my_utility.spell_delays.condemn;
            console.print("Condemn (Rotation)");
        end
        return true;
    end;
