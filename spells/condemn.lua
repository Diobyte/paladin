...
<<<<<<< Updated upstream
    elites_only         = checkbox:new(false, get_hash(my_utility.plugin_label .. "condemn_elites_only")),
    cast_delay          = slider_float:new(0.01, 1.0, 0.1, get_hash(my_utility.plugin_label .. "condemn_cast_delay")),
=======
<<<<<<< Updated upstream
    min_enemy_count     = slider_int:new(1, 10, 3, get_hash(my_utility.plugin_label .. "condemn_min_enemy_count")),
    elites_only         = checkbox:new(false, get_hash(my_utility.plugin_label .. "condemn_elites_only")),
    cast_delay          = slider_float:new(0.01, 1.0, 0.1, get_hash(my_utility.plugin_label .. "condemn_cast_delay")),
    is_independent      = checkbox:new(false, get_hash(my_utility.plugin_label .. "condemn_is_independent")),
=======
    force_priority   = checkbox:new(true, get_hash(my_utility.plugin_label .. "condemn_force_priority")),
    elites_only      = checkbox:new(false, get_hash(my_utility.plugin_label .. "condemn_elites_only")),
    cast_delay       = slider_float:new(0.01, 1.0, 0.1, get_hash(my_utility.plugin_label .. "condemn_cast_delay")),
>>>>>>> Stashed changes
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
    -- Check enemy count
    local enemy_count = my_utility.enemy_count_simple(8.0)
    if enemy_count < menu_elements.min_enemy_count:get() and not target:is_boss() and not target:is_elite() then
        return false
    end

    -- Suicide Prevention: Don't pull enemies if HP is critical and no defensive buffs
    local local_player = get_local_player()
    if local_player then
        local current_hp_pct = local_player:get_current_health() / local_player:get_max_health()
        if current_hp_pct < 0.25 then
            -- Check for defensive buffs (Aegis, Fortress, etc.)
            -- Assuming we don't have easy access to check all buffs here, we'll just be safe
            -- Unless we are in a "do or die" situation, pulling more aggro is bad
            return false
        end
    end
