...
local menu_elements = {
    ...
    debug_mode = checkbox:new(false, get_hash("paladin_rotation_holy_light_aura_debug_mode")),
}
...
local function logics()
    ...
    if not menu_elements.main_boolean:get() then 
        ...
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Disabled") end
        return false, 0 
    end
    ...
    if not is_logic_allowed then 
        is_aura_active = false
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Spell not allowed") end
        return false, 0
    end
    ...
    if not is_in_combat() then
        is_aura_active = false
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Not in combat") end
        return false, 0
    end
    ...
    if has_buff and buff_remaining > BUFFER_TIME then
        is_aura_active = true
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Buff active, remaining: " .. string.format("%.1f", buff_remaining)) end
    ...
    if not should_recast and not is_expiring and last_cast_time > 0 then
        is_aura_active = true
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Still active by timing") end
        return false, 0
    end
    
    if now < next_time_allowed_cast then 
        is_aura_active = false
        if debug_enabled then console.print("[HOLY_LIGHT DEBUG] On cooldown") end
        return false, 0
    end
    ...
    is_aura_active = false
    if debug_enabled then console.print("[HOLY_LIGHT DEBUG] Cast failed") end
    return false, 0
end
