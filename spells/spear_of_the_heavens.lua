...
local menu_elements =
{
    ...
    is_independent      = checkbox:new(false, get_hash(my_utility.plugin_label .. "spear_of_the_heavens_is_independent")),
}
...
local function logics(target)
    ...
    local predicted_position = prediction.get_future_unit_position(target, 0.4) -- 0.4s delay estimate
    if predicted_position then
        cast_position = predicted_position
    end

    if cast_spell.position(spell_data.spear_of_the_heavens.spell_id, cast_position, 0) then
