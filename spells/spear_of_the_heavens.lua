...
local menu_elements = {
    ...
    min_cooldown = slider_float:new(0.0, 10.0, 0.5, get_hash("paladin_rotation_spear_of_the_heavens_min_cd")),
    enemy_type_filter = combo_box:new(0, {"All", "Elite+", "Boss"}, get_hash("paladin_rotation_spear_of_the_heavens_enemy_type")),
...
local function logics(best_target, area_analysis)
    ...
    if prediction and prediction.get_future_unit_position then
        local predicted_pos = prediction.get_future_unit_position(target, 0.3) -- 0.3s delay approx
